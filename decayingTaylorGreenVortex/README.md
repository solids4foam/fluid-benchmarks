...
- **Static 3**: starts with the mesh from Static 1 and applies smooth distort
  internally *and* at the boundary using a sinuisoidal function. This type of
...
- $\mathbf{B} = (B_x, B_y, B_z)$ and $\mathbf{C} = (C_x, C_y, C_z)$:
  frequency vectors
    ...
    This is due to the fact that partial derivatives of the function is
    non-zero at some points on the boundary.
